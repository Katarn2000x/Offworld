...
**********************************************************************/
class OWGFxMenu extends GFxMoviePlayer
    config(UI);
...
var OWGFxMenu_MainMenu MainMenuView;

//var OWGFxMenu_Options OptionsMenuView;
 ...
 */
var array<OWGFxMenu_View>		ViewStack;
...
{
	super.Start();
	`Log("Started the main menu");
	Advance(0);

	if (!bInitialized)
	{
		`Log("Not initialized, start config");
		ConfigFrontEnd();
	}
	...
	`Log("Pushing the main menu view");
	PushViewByName('MainMenu');
	return TRUE;
}
...
final function LoadViews()
{
	...
	`Log("Loading views");
	`Log("This many views are in ViewData: " $ViewData.Length);
	
	for (i = 0; i < ViewData.Length; i++) 
	{
		`Log("loading the viewdata: ");
		`Log("ViewName: " $ViewData[i].ViewName);
		`Log("SWFName: " $ViewData[i].SWFName);
		LoadView( ViewData[i] );
	}
}
...
final function LoadView(ViewInfo InViewInfo)
{
	...
	`Log("Loading view: " $InViewInfo.SWFName);
 ...
 */ 
final function PushViewByName(name TargetViewName, optional OWGFxMenu_Screen ParentView)
{
	...
	switch (TargetViewName)
	{
		...
		/*
		case( 'Options' ):
			ConfigureTargetView( OptionsActionView );
			break;
		case ( 'InstantAction' ): 
 ...
 */
function ConfigureTargetDialog(coerce OWGFxMenu_View TargetDialog)
{
 ...
 */
function ConfigureTargetView(OWGFxMenu_View TargetView)
{
    if( IsViewAllowed( TargetView ) )
    {
		`Log("Configured target view: " $TargetView);
...
function bool IsViewAllowed(OWGFxMenu_View TargetView)
{
    local byte i;	
    local name TargetViewName;

	`Log("Checking if view is allowed");
    ...
    if ( TargetView == none )
    {
		...
		`Log("View is not allowed, target view is null");		
        return false;
    }
    ...
    for ( i = 0; i < ViewStack.Length; i++ )
    {
        if (ViewStack[i].ViewName == TargetViewName)
        {
			...
			`Log("View is not allowed, target view is already on the stack");					
            return false;
        }
    }
	`Log("View is allowed");
...
function PushView(coerce OWGFxMenu_View targetView) 
{     
    ActionScriptVoid("pushStandardView"); 
}

/** AS stub for pushing a view onto the stack. */
function PushDialogView(coerce OWGFxMenu_View dialogView) 
{
...
final function ConfigureView(OWGFxMenu_View InView, name WidgetName, name WidgetPath)
{	
...
{    
    ...
    {           
        case ('MainMenu'):
            ...
            {
				`Log("Main menu widget is being initialized");
                MainMenuView = OWGFxMenu_MainMenu(Widget);
...
defaultproperties
{    
	WidgetBindings.Add((WidgetName="MainMenu",WidgetClass=class'OWGFxMenu_MainMenu'))
	//WidgetBindings.Add((WidgetName="Options",WIdgetClass=class'OWGFxMenu_Options'))
	...
	*/
    ...
    TimingMode=TM_Real
    bInitialized=FALSE
	MovieInfo=SwfMovie'OWFrontEnd.ow_menu_manager'
